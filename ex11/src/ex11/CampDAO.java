...
public class CampDAO {
	...
	//새로운 캠핑번호 생성
	public String getNo() {
		String cno =""; //캠핑장번호 자동 설정
		try {
			String sql="select max(cno)+1 nno from camp"; //캠핑장번호의 최대값+1로 설정 
			PreparedStatement ps=con.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
			if(rs.next()) cno=rs.getString("nno");// 새로운 컬럼명인 nno 값을 cno 변수에 대입
		}catch(Exception e) {
			System.out.println("새로운 캠핑번호 생성 오류:" + e.toString());
		}
		return cno; //캠핑장 번호(문자열) 리턴
	}
